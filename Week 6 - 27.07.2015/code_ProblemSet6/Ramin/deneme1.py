...
def applyCoder(text, coder):
    ...
    coded_text=''
    for a in text:
        if a in string.punctuation or a in string.digits or a==' ':
            coded_text+=a
        else:
            coded_text+=coder[a]
    return coded_text
...
#
cipher=8
filen=open('encrypted_code.txt','a')
tex=open('Aytenemektub.txt','r')
textn=tex.read()
encrypted_text=applyShift(textn,cipher)
filen.write(encrypted_text)
print 'it is ready'
filen.close()
tex.close()

filen=open('decrypted_code.txt','w')
tex=open('encrypted_code.txt','r')
textn=tex.read()
decrypted_text=applyShift(textn,26-cipher)
filen.write(decrypted_text)
print 'it is ready'
filen.close()
tex.close()
